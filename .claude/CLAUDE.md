...
jobs. Designed for DX-first development.
...
Alternatively, rely on TypeScript error messages and existing type imports in the codebase rather
...
Do not write tests that only verify type-level behavior (e.g., `expectTypeOf`, `type-fest` helpers,
assignability checks). Types are validated by `tsc` — testing them adds no value.
