...
2. Calls `engine.start()`, which returns a `Promise<StartupResult>` that resolves after planner
   cache load, startup recovery, and the first IssuePoller and SpecPoller cycles both complete. The
   TUI shows a centered loading spinner with "Starting…" text until the Promise resolves. The
   three-pane layout is not rendered during startup.
