...
The engine uses two independent pollers to monitor GitHub for state changes. Each poller runs on its
own interval, maintains its own snapshot, and reports results to the Engine Core. Pollers are pure
sensors — they detect state changes and report them. They do not make dispatch decisions.
...
- The IssuePoller only tracks `task:implement` issues — `task:refinement` issues are outside the
...
1. Query open issues with the `task:implement` label via `GitHubClient`. Only `task:implement`
   issues are tracked — `task:refinement` issues are outside the control plane's scope (they do not
   have status transitions that drive agent dispatch).
...
invocation, not via the interval timer. It awaits both first cycles before resolving. Interval-based
...
empty. This enables the Planner Cache (see `control-plane-engine-planner-cache.md`) to prevent
...
non-null at cache write time. See `control-plane-engine-planner-cache.md` for the write
precondition.
...
- [ ] Given the IssuePoller is running, when its poll interval elapses, then it queries GitHub
      Issues independently of the SpecPoller.
...
- [ ] Given the IssuePoller encounters a GitHub API error, when the error occurs, then the
      SpecPoller continues operating on its own interval.
...
- **SpecPoller commit SHA is HEAD, not per-file.** The `commitSHA` in `SpecPollerBatchResult` is the
  HEAD commit of the default branch at poll time, not the specific commit that modified each spec
  file. If multiple commits were pushed between poll cycles, the diff URL shows the HEAD commit's
  full diff, not a per-file change view. Acceptable because the notification identifies the changed
  file path, giving the user enough context to find the relevant changes.

## Dependencies

- `control-plane-engine.md` — Parent engine spec (architecture, GitHubClient, event types, dispatch
  logic, configuration)
- `control-plane-engine-planner-cache.md` — Planner Cache (provides initial snapshot seed to
  SpecPoller)
...
- [control-plane-engine.md: Configuration](./control-plane-engine.md#configuration) — IssuePoller
  and SpecPoller configuration settings
