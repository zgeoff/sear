...
test('it renders both panes on initial startup', async () => {
...
test('it toggles focus back when Tab is pressed again', async () => {
  const { lastFrame, stdin, emit } = await setupStartedTest();
  ...
  // Move focus back to task list
  stdin.write('\t');

  // Allow focus change to take effect
  await new Promise((r) => setTimeout(r, 50));

  // Pressing j should change issue selection since focus is back on task list
  stdin.write('j');
...
test('it computes equal pane widths when terminal width divides evenly', () => {
  // 2 panes + 3 border columns = 82 → 79 content / 2 = 39 each + 1 remainder
  const widths = computePaneWidths(83);
  expect(widths[0]).toBe(40);
  expect(widths[1]).toBe(40);
  expect(widths[0] + widths[1] + 3).toBe(83);
});

test('it allocates remainder columns to the rightmost pane', () => {
  const widths = computePaneWidths(80);
  expect(widths[0]).toBe(38);
  expect(widths[1]).toBe(39);
  expect(widths[0] + widths[1] + 3).toBe(80);
});

test('it accounts for exactly three border columns', () => {
  const widths = computePaneWidths(120);
  expect(widths[0] + widths[1] + 3).toBe(120);
});
